...
int main(int argc, char *argv[]){
    ...
    { 
        ...
        else if(MODE == 1)
        {
            ...
            textprintf_ex(buffer, font, 235,10, makecol(255,255,255), -1, "Level: %d", level+1);
            ...
            if (player[1].life > 0)
            {
        		...
        		while(speed_counter > 0)
                {
                   ...
                   //Player two controls
                    if(!player[1].dead())
                    {
                		if(key[KEY_RIGHT] && !key[KEY_LEFT] && player[1].x < map_scroll+500-player[1].c)
                        {
                    ...
                    if(enemy_counter == 0)
                    {
                        ...
                        if(conjuration_num == 0)
                        {
                            ...
                            if (epos <= 450)
                            {
                                for(int i = 0; i < 4; i++)
                                {
                                     enemy[current_enemy].setup(4,epos+i*50, -50-i*50,0,1.5,5);
                                     current_enemy++;
                                }
                            }
                            else
                        ...
                        else if(conjuration_num == 1)
                        {
                            ...
                            {
                                 enemy[current_enemy].setup(4,rand()%750+225, -50,0,1.5,rand()%6);
                                 current_enemy++;
                            }
                        }
                        else if(conjuration_num == 2)
                        {
                            ...
                            for(int i = 0; i < 4; i++)
                            {
                                 enemy[current_enemy].setup(4,epos+i*50,-50,0,1.5,4);
                                 current_enemy++;
                            }
                        }
                        else if(conjuration_num == 3)
                        {
                            ...
                            for(int i = 0; i < 4; i++)
                            {
                                 enemy[current_enemy].setup(4,epos+i*50, -50,0,1.5,rand()%6);
                                 current_enemy++;
                            }
                        }
                        ...
                        {
                            epos = rand()%500+200;
                            enemy[current_enemy].setup(2,epos, -50,0,1.5,5);
                            current_enemy++;
                            for(int i = 1; i < 4; i++)
                            {
                                 enemy[current_enemy].setup(2,epos-50*i, -50-i*60,0,1.5,5);
                                 enemy[current_enemy+1].setup(2,epos+50*i, -50-i*60,0,1.5,5);
                                 current_enemy+=2;
                            }
                        }
                        else if(conjuration_num == 5)
                        ...
                        else if(conjuration_num == 6)
                        {
                            for(int i = 0; i < 6; i++)
                            {
                                 enemy[current_enemy].setup(4,rand()%750+225, -50,0,1.5,6);
                                 current_enemy++;
                            }
                        }
                        else
                    ...
                    else if(level_counter >= level_length && conjur_boss == TRUE)
                    {
                         ...
                         boss_alive = TRUE;
                         spawn_minion = true;
                            enemy[current_enemy].setup(20,rand()%500, -499,0,1.5,7);
                              current_enemy++;
                    ...
                    if (conjuration_num == 5)
                    {
                        if(enemy_counter > 70-difficulty)
                        {
                            enemy_counter = 0;
                        }
                    }
                    ...
                    {
                        if(enemy_counter > 200-difficulty2)
                        {
                            enemy_counter = 0;
                        }
                    }    
        ...
        //WARNING-----INEFFICIENT CODING BELOW-----------------------------------------------------------------------------------------------------------
                ...
                {
                    //Bullet Impact
                    if(bullet[i].impact == true)
                    {
                         if(impact_num < 1)
                             draw_sprite(buffer,impact,bullet[i].x()-map_scroll-37+8, bullet[i].y());
                         if(impact_num >= 1)
                             bullet[i].impact = false;
                    }
                    //Animations
                    if(blowup[i].show == true)
                    {
                         ...
                         else if(blowup[i]. type == 1)
                         {
                             ...
                             if(blowup[i].counter == 2 || blowup[i].counter == 33 || blowup[i].counter == 64 || blowup[i].counter ==  95)
                             {
                                 for(int j = 0; j < 5; j++)
                                 {
                                     blowup[animation_counter].setup(blowup[i].x+rand()%500,blowup[i].y+rand()%400,0);
                                     animation_counter++;
                                 }
                             }
                    ...
                    if (enemy[i].ready())
                    {
                        ...
                        if (enemy[i].pj_x > 1100+50 || enemy[i].pj_x < -50 || enemy[i].pj_y > 750+50 || enemy[i].pj_y < -50)
                           enemy[i].NOTshoot();
                        if (enemy[i].x()>1100+50 || enemy[i].x() < -50 || enemy[i].y() > 750+50 || enemy[i].y() < -50)
                            enemy[i].insta_death();
                        for (int j = 0; j<2;j++)
                        {
                            if(!player[j].dead())
                    ...
                    if(!enemy[i].dead())
                    {
                        ...
                        for (int j = 0; j < maxbullets; j++)
                        {
                            if (bullet[j].ready())
                            {
                                if(bulletcollide(bullet[j].x()-map_scroll, bullet[j].y(), 16, 20, enemy[i].x()-map_scroll, enemy[i].y(), enemyp[enemy[i].type]->w,enemyp[enemy[i].type]->h,enemyp[enemy[i].type]))
                                 {
                                     bullet[j].NOTshoot();
                                     enemy[i].hurt();
                                     if(enemy[i].type != 7)
                ...
                for(int i = 0; i < 2; i ++)
                {
                    ...
                    {        
                        if (player[i].FFshow == true)
                        {
                           ...
                           if (FFframe_counterP[i] >= 3 && FFframe_counter[i] < 6)
...
void UpgradeScreen(int number_of_players,int damage[],int speed[],int life[], int bomb[], int shield[], BITMAP *background)
{
     ...
     while(Loop==0){
         while(speed_counter > 0)
         {
             ...
             if(!key[KEY_U] && !key[KEY_S] && !key[KEY_W])
             {
                 keyhold[0] = FALSE;
             }
             for(int i = 0; i<2;i++)
...
{  
    ...
    if(keyrel(L) || (key[L] && key[R] && Lcheck == true))
    {
        turn = 1;
        Lcheck = false;
    }
    ...
    if (keyrel(R) || (key[L] && key[R] && Rcheck == true))
    {
       turn = 2;
       Rcheck = false;
    }
