//--------------------------------------------------------------\\
//____   ____     .__    .___   _________  __                   \\
//\   \ /   /____ |__| __| _/  /   _____/_/  |_   ____ ______   \\
// \   Y   //  _ \|  |/ __ |   \_____  \ \   __\_/ __ \\____ \  \\
//  \     /(  <_> )  / /_/ |   /        \ |  |  \  ___/|  |_> > \\
//   \___/  \____/|__\____ |  /_______  / |__|   \___  >   __/  \\
//                        \/          \/             \/|__|     \\
//--------------------------------------------------------------\\
//Production of Zuqi Robert L. & Darren H.
//ICS3U Copyright @ 2012 | 
//Mrs. Odecki 

//For more info please refer to the external documentation.
#include<allegro.h>
#include<stdio.h>
#include<iostream>
#include<math.h>
...
class jet
{
    public:
        ...
        int life_counter;
        int bomb_num;                              //Number of Bombs that the player has left
        int tgm_counter;                           //Toggle godmode counter
        ...
        void hurt(){HP --;}                        //When player gets hit         
        void insta_death(){HP = 0;}
...
int main(int argc, char *argv[]){
    ...
    while(MODE != 2)
    {
        ...
        else if(MODE == 1)
        {
            ...
            //Initializationz for the start of each level
            ...
            //open the gatez and check for initiaization
            ...
            if(bgy[1] > 1919)
                bgy[1] = -1919;
            //Player Speed and life
            for(int i = 0; i <2;i++)
            {
                player[i].speed = speed + dex[i];
                player[i].life = life[i];
            }
            //Prints out the informations
            ...
            if (player[1].life > 0)
            {
        		...
        		while(speed_counter > 0)
                {
                   ...
                   //Player two controls
                    ...
                    enemy_counter++;
                    //Snaps camera to other player one player dies
        ...
        //Calculations---------------------------------------------------------------------------------------------
                for(int i = 0; i < 1000; i++)
                {
                    ...
                    if(!enemy[i].dead())
                    {
                        ...
                        //Bullet Calculations and Collisions
                ...
                for(int i = 0; i < 2; i ++)
                {
                    ...
                    if(player[i].life > 0 && player[i].dead())
                    {
                        ...
                        {
                            //resests the player y position and hp
                            player[i].HP = shield[i];
                            player[i].y = 650;
...
void DisplayMenu(int &Num,int &Num_Player)
{
     ...
     int mode = 0;//Different menu8 screens
     int select = 0;//selection
     int keyhold = FALSE;
     int Loop = 0;//getting out of menu
     int ani_counter = 0; //ship animation
     pos cursor;
...
//Sets the bullet existance to false
void Bullet::NOTshoot(){
    be = 0;
}
//Sets the bullet existance to true
void Bullet::shoot(){
    be = 1;
}
//Movement of the bullet and bullet boundary
